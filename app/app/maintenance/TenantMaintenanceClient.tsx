...
export default function TenantMaintenanceClient() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (photoFile) {
        const path = `maintenance/${Date.now()}-${photoFile.name.replace(/\s/g, '_')}`
        ...
        {showForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-5" style={{ background: 'rgba(0,0,0,.6)', backdropFilter: 'blur(6px)' }} onClick={() => setShowForm(false)}>
            ...
            >
              ...
              {/* Photo (optional) */}
              <div className="mb-6">
                <label className="block text-[11.5px] font-extrabold uppercase tracking-wider mb-2" style={{ color: '#6B7280' }}>Photo (optionnel)</label>
                <input
                  type="file"
                  accept="image/*"
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      {req.photo_url && (
                        <img src={req.photo_url} alt={req.title} className="mt-2 rounded-[10px] object-cover" style={{ width: '120px', height: '90px' }} />
                      )}
                    </div>
                  ...
                  {req.bailleur_comment && (
                    <div className="mt-3 p-3 rounded-[10px]" style={{ background: '#F9FAFB', border: '1px solid #F3F4F6' }}>
                      <div className="text-[10.5px] font-bold uppercase tracking-wider mb-1" style={{ color: '#9CA3AF' }}>Réponse du propriétaire</div>
