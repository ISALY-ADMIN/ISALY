...
/**
 * [HIDDEN - REFONTE ACCUEIL] Ancienne page d'accueil publique.
 *
 * Ce fichier EST l'ancien app/page.tsx, déplacé tel quel (git mv, historique
 * conservé) le jour de la refonte de l'accueil. Rien n'y a été supprimé : seul
 * le `'use client'` d'origine et le nom du composant ont changé, pour qu'il
 * puisse coexister avec la nouvelle page sans collision.
 *
 * Il n'est plus monté par aucune route. Pour le remettre en ligne, il suffit de
 * le rendre depuis app/page.tsx à la place de <HomeClient />.
 *
 * Ce qui en a été REPRIS dans la nouvelle page, et ne doit pas se perdre si on
 * touche à ce fichier :
 *   · le logo /LOGO_ISALY.png en en-tête et en pied de page ;
 *   · les liens légaux (CGU, Confidentialité, Contact) ;
 *   · <CookieSettingsLink /> — réouverture du bandeau de consentement, RGPD.
 * Les métadonnées SEO, elles, n'ont jamais vécu ici : elles sont dans
 * app/layout.tsx et n'ont pas bougé.
 */
